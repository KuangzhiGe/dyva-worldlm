...
class HFCausalLLMBackbone(LLMBackbone, ABC):
    def __init__(
        ...
        # print(f"Loading from: {hf_hub_path}")
        ...
        # [Contract] `inference_mode` means we're loading from a pretrained checkpoint; no need to load base weights!
            ...
            if llm_family == "llama2":
                overwatch.info("Loading LLaMa-2 LLM from local path", ctx_level=2)
                # print("Loading LLaMa-2 LLM from local path")
                llm_config = AutoConfig.from_pretrained(hf_hub_path, token=hf_token, local_files_only=True)
                overwatch.info("Loading LLaMa-2 LLM from loaded config", ctx_level=2)
                # print("Loading LLaMa-2 LLM from loaded config")
