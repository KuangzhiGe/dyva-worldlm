...
LLAMA2_MODELS = {
    ...
    "vicuna-v15-7b": {
        "llm_family": "llama2", "llm_cls": LlamaForCausalLM, "hf_hub_path": "/mnt/world_foundational_model/gkz/ckpts/hub/models--lmsys--vicuna-7b-v1.5/snapshots/3321f76e3f527bd14065daf69dad9344000a201d" #"lmsys/vicuna-7b-v1.5"
    },
...
class LLaMa2LLMBackbone(HFCausalLLMBackbone):
    def __init__(
        ...
        if self.tokenizer.pad_token is None:
            # print(f"Loading LLaMa-2 LLM from local path")
            self.tokenizer.add_special_tokens({"pad_token": "<PAD>"})
            self.llm.config.pad_token_id = self.tokenizer.pad_token_id
            self.llm.resize_token_embeddings(len(self.tokenizer), pad_to_multiple_of=64)
        else:
            # print("Found tokenizer padding token")
            self.llm.config.pad_token_id = self.tokenizer.pad_token_id
