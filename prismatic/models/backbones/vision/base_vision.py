...
class TimmViTBackbone(VisionBackbone, ABC):
    def __init__(
        ...
        if self.override_act_layer is None:
            self.featurizer: VisionTransformer = self._create_model_with_retry(
                self.timm_path_or_url
            )
        else:
            self.featurizer: VisionTransformer = self._create_model_with_retry_act(
                self.timm_path_or_url
            )
    ...
    def _create_model_with_retry(self, url: str) -> VisionTransformer:
        ...
        return model
    def _create_model_with_retry_act(self, url: str) -> VisionTransformer:
        """使用tenacity进行带重试的模型创建。"""
        model: VisionTransformer = timm.create_model(
            url,
            pretrained=True,
