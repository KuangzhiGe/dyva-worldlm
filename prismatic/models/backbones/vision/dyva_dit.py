...
from prismatic.models.backbones.vision.base_vision import (ImageTransform,
                                                            VisionBackbone,
                                                            unpack_tuple)
from prismatic.models.backbones.vision.dyva_cogvideox import CogVideoXVisionBackbone
...
SIGLIPCOGVIDEOX_VISION_BACKBONES = {
    "dyva_dit_siglip": {
        "siglip": "vit_so400m_patch14_siglip_224",
        "cogvideox_height": 448,
        "cogvideox_width": 448,
    },
    "dyva_dit_dino": {
...
class DyVACogVideoXVisionBackbone(VisionBackbone):
    """
    ...
    def __init__(self,
        ...
        if self.vision_backbone_id not in SIGLIPCOGVIDEOX_VISION_BACKBONES:
            raise ValueError(f"Vision backbone ID `{self.vision_backbone_id}` is not defined in SIGLIPSVD_VISION_BACKBONES.")

        model_cfg = SIGLIPCOGVIDEOX_VISION_BACKBONES[self.vision_backbone_id]
        svd_height = model_cfg["cogvideox_height"]
        svd_width = model_cfg["cogvideox_width"]
        ...
        svd_featurizer = CogVideoXVisionBackbone(
            vision_backbone_id="dyva-cogvideox",
    ...
    def get_fsdp_wrapping_policy(self) -> Callable:
        ...
        svd_wrap_policy = partial(_module_wrap_policy, module_classes={CogVideoXVisionBackbone})
