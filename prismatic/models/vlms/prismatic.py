...
from prismatic.util.nn_utils import FusedMLPProjector, LinearProjector, MLPProjector, SVDProjector, DualProjector
...
class PrismaticVLM(VLM):
    def __init__(
        ...
        torch.manual_seed(vision_backbone.embed_dim if isinstance(vision_backbone.embed_dim, int) else vision_backbone.embed_dim["svd"])
        ...
        elif "dual" in arch_specifier:
            self.projector = DualProjector(vision_backbone.embed_dim, llm_backbone.embed_dim)
        ...
        # Whether to align svd feature with llm input embedding and hyperparameter for weight
        self.align_svd_with_llm = False
    ...
    def forward(
        ...
        # overwatch.info(f"Image sizes: {image_sizes}")
    ...
    @torch.inference_mode()
    def generate(self, image: Union[Image.Image, List[Image.Image]], prompt_text: str, **kwargs: str) -> str:
        # For now, only support generation with a batch size of 1 for simplicity
