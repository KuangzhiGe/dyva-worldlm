...
from typing import Callable, Dict, List, Optional, Type, Union # type: ignore
...
from prismatic.util.nn_utils import FusedMLPProjector, LinearProjector, MLPProjector, SVDProjector
...
class WM_VLM(VLM):
    def __init__(
        ...
        elif "svd" in arch_specifier:
            self.projector = SVDProjector(vision_backbone.embed_dim, llm_backbone.embed_dim)
        else:
        ...
        # Load DINOSigLIP
        pretrained_checkpoint = Path("/mnt/world_foundational_model/gkz/ckpts/hub/models--TRI-ML--prismatic-vlms/snapshots/a3ba8a19c453a82eaf5a3fb1e699dd9e441f0a12/prism-dinosiglip+7b/checkpoints/latest-checkpoint.pt")
        model_state_dict_proj = torch.load(pretrained_checkpoint, map_location="cpu")["model"]
        assert (
            "projector" in model_state_dict_proj and "llm_backbone" in model_state_dict_proj
        ), "PrismaticVLM `from_pretrained` expects checkpoint with keys for `projector` AND `llm_backbone`!"
        self.dinosiglip_projector = FusedMLPProjector(2176, llm_backbone.embed_dim)
        self.dinosiglip_projector.load_state_dict(model_state_dict_proj["projector"])
        from prismatic.models.materialize import get_vision_backbone_and_transform
        self.dinosiglip, _ = get_vision_backbone_and_transform(
            "dinosiglip-vit-so-224px",
            "resize-naive",
        )
        self.dinosiglip.requires_grad_(False)
        self.dinosiglip_projector.requires_grad_(False)
        self.dinosiglip.eval()
        self.dinosiglip_projector.eval()
    ...
    def from_pretrained(
        ...
        # print(model_id)
        if "svd" in model_id:
            overwatch.info(f"Loading Model_ID: {model_id} from Checkpoint path `{pretrained_checkpoint}`")
            ...
            ), "WM `from_pretrained` expects checkpoint with keys for `projector` AND `llm_backbone`!"
            vlm.llm_backbone.load_state_dict(model_state_dict["llm_backbone"])
            
            # pretrained_checkpoint = Path("/mnt/world_foundational_model/gkz/prismatic-vlms/runs/siglipsvd_align/checkpoints/latest-checkpoint.pt")
            # model_state_dict_proj = torch.load(pretrained_checkpoint, map_location="cpu")["model"]
    ...
    def forward(
        ...
        if isinstance(pixel_values, dict):
            dinosiglip_values = {"dino": pixel_values["dino"][multimodal_indices], "siglip": pixel_values["siglip"][multimodal_indices]}
            pixel_values = pixel_values["svd"][multimodal_indices]
        ...
        with torch.set_grad_enabled(False):
            if isinstance(dinosiglip_values, dict):
                dinosiglip_patch_features = self.dinosiglip({k: dinosiglip_values[k][multimodal_indices] for k in dinosiglip_values})
            else:
                dinosiglip_patch_features = self.dinosiglip(dinosiglip_values[multimodal_indices])
        # overwatch.info(f"[DEBUG] `forward`方法中，编码后图像特征 (patch_features) 的形状: {patch_features.shape}")
        # Projection Logic :: [bsz, num_patches, llm_embed_dim] =>> num_patches = (2 *) (256 + 1) for ViT-L + CLS
        projected_dinosiglip_patch_embeddings = self.dinosiglip_projector(dinosiglip_patch_features)
        projected_patch_embeddings = self.projector(patch_features)
        projected_patch_embeddings = torch.cat([projected_dinosiglip_patch_embeddings, projected_patch_embeddings], dim=1)
    ...
    def generate(self, image: Image, prompt_text: str, **kwargs: str) -> str:
        ...
        #     pil_image_to_save = to_pil(tensor)
        #     save_path = f"{name}_ts.png"
