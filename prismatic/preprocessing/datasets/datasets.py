...
from transformers import CodeGenTokenizerFast, LlamaTokenizerFast, PreTrainedTokenizerBase, Qwen2TokenizerFast
...
class FinetuneDataset(Dataset[Dict[str, torch.Tensor]]):
    def __init__(
        ...
        # NOTE: Prevent Invalid Image Paths
        original_sample_num = len(self.examples)
        valid_examples = []
        for ex in tqdm(self.examples):
            if "image" in ex:
                if isinstance(ex["image"], list):
                    all_exist = all((self.image_dir / Path(p)).exists() for p in ex["image"])
                    if all_exist:
                        valid_examples.append(ex)
                    else:
                        print(f"[Warning] Missing one or more images in {ex['image']}, skipping sample")
                else:
                    if (self.image_dir / Path(ex["image"])).exists():
                        valid_examples.append(ex)
                    else:
                        print(f"[Warning] Missing image {ex['image']}, skipping sample")
            else:
                valid_examples.append(ex)
        self.examples = valid_examples
        # Print Dataset Summary
        print(f"[Info] Loaded {len(self.examples)} / {original_sample_num} valid examples.")
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        for turn_idx, turn in enumerate(conversation):
            ...
            # Phi-2 Tokenizer == CodeGenTokenizer (Fast) -- no special handling!
                ...
                # Qwen 的分词器和 LLaMA 不一样，不需要处理空格问题
                # 保持原样即可
                pass
        ...
        if "image" in self.examples[idx]:
            # modified by ssj
            # Process Image --> get "pixel_values" (will either be a torch.Tensor OR a Dict[str,torch.Tensor])
            if isinstance(self.examples[idx]["image"], list):
                image_paths = [Path(im_path) for im_path in self.examples[idx]["image"]]
                images = [Image.open(self.image_dir / image_path).convert("RGB") for image_path in image_paths]
                # Process Image --> get "pixel_values" (will either be a torch.Tensor OR a Dict[str,torch.Tensor])
