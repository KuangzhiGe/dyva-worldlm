...
class FSDPStrategy(TrainingStrategy):
    ...
    def run_setup(self, run_dir: Path, n_train_examples: int) -> None:
        ...
        # vision_half_dtype = self.vlm.vision_backbone.half_precision_dtype
        # mp_dtype = self.mixed_precision_dtype
        # overwatch.info(f"Vision Backbone Half Precision Dtype: {vision_half_dtype} | FSDP Mixed Precision Dtype: {mp_dtype}")
        ...
        if self.lr_scheduler_type == "linear-warmup+cosine-decay":
            ...
            #   => Create Parameter Groups with separate LR for UNet
            #   => Bias terms and normalization layers shouldn't be decayed!
            # Initialize 4 groups:
            non_unet_decay = []        # Non-UNet, needs weight decay (original LR)
            non_unet_no_decay = []     # Non-UNet, no weight decay (original LR)
            unet_decay = []            # UNet, needs weight decay (1/10 LR)
            unet_no_decay = []         # UNet, no weight decay (1/10 LR)

            for name, param in self.vlm.named_parameters():
                if not param.requires_grad:
                    continue  # Skip frozen parameters

                # Check if parameter belongs to UNet (adjust pattern based on your model's naming!)
                is_unet_param = "unet." in name  # Modify this if your UNet uses a different naming pattern

                # Check if parameter should be decayed
                if param.ndim <= 1 or name.endswith(".bias"):
                    # No weight decay
                    if is_unet_param:
                        unet_no_decay.append(param)
                    else:
                        non_unet_no_decay.append(param)
                else:
                    # Needs weight decay
                    if is_unet_param:
                        unet_decay.append(param)
                    else:
                        non_unet_decay.append(param)

            # Define original LR and UNet LR (1/5 of original)
            original_lr = self.learning_rate
            unet_lr = original_lr / 5

            # Build Parameter Groups with appropriate LR and weight decay
            groups = [
                # Non-UNet groups (original LR)
                {"params": non_unet_decay, "weight_decay": self.weight_decay, "lr": original_lr},
                {"params": non_unet_no_decay, "weight_decay": 0.0, "lr": original_lr},
                # UNet groups (1/10 LR)
                {"params": unet_decay, "weight_decay": self.weight_decay, "lr": unet_lr},
                {"params": unet_no_decay, "weight_decay": 0.0, "lr": unet_lr},
            ]

            # Create Optimizer & LR Scheduler
            self.optimizer = AdamW(groups)  # LR is already set in groups, no need to pass here
            ...
            # Note: Removed the loop that set all LR to 0.0 - LR is now controlled by group definitions
            # The scheduler will handle warmup/decay based on the group-specific base LRs
